...
import { YearsDropdown } from "../../../components/Reusable/Calendar";
...
export default function Policy() {
  ...
  const [FinanceReference, setFinanceReference] = useState<FinanceReference[]>(
    []
  );
    ...
    <div className="py-20 640px:py-10">
      ...
      >
        ...
        <li className="border-l pl-10 ml-10 640px:m-0 640px:p-0 640px:border-none w-full overflow-auto">
          <h4 className="main_text noMB hidden 640px:block">FINANCE PERIOD</h4>
          <section className="flex items-center mb-10 ">
            <SelectDropdown
              ...
              inputElement={
                <input
                  className={`w-full field ${!isToggle && "noStyle"} `}
                  value={isMonth}
                  readOnly
                  autoComplete="off"
                />
          ...
          <section className=" 640px:mt-5">
            <table className="w-full">
              ...
              <tbody>
                {FinanceReference?.map((item: FinanceReference, index) => (
                  <tr key={index}>
                    <td className="py-2 pr-5 min-w-[200px]">
                      <h4 className="main_text noMB">{item.document}</h4>
                    </td>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.value.length <= 9) {
                              UpdateValue("prefix", e.target.value, item.id);
                            }
                          }}
                        />
                      ) : (
