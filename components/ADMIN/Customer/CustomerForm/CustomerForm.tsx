...
export default function CustomerForm({ DefaultValue }: Props) {
  ...
  const CreateHandler = async (button: string) => {
    ...
    let Payload: any = {
      ...
      individual_birth_date: isValid(date) ? format(date, "yyyy-MM-dd") : "",
      image_photo:
        isCustomerForm.image_photo === null ? "" : isCustomerForm.image_photo,
      image_valid_id:
  ...
  return (
    <ModalTemp>
      ...
      {FormPage === "property" && (
        <h1 className={style.modal_label_primary}>Property Informations</h1>
      )}
      {FormPage === "primary" && (
        <div className=" w-[95%] text-[12px] flex items-center justify-between mb-5">
          <aside className=" w-4/12 480px:w-2/4">
            <p className=" text-[12px] font-semibold mb-1 w-[90%]">TYPE</p>
            <SelectDropdown
            ...
            {isCustomerForm.type === "" && (
              <label className=" font-NHU-bold">Please Select a Type</label>
            )}
          </aside>
      ...
      {isCustomerForm.type !== "" &&
        (FormPage === "primary" || FormPage === "contact-information") && (
      ...
      <div className={`${FormPage !== "property" && "hidden"}`}>
        <CustomerUnitCodeForm
          ...
          MutateLoadingDraft={
            router.query.draft === undefined ? MutateDraftLoading : loadingDraft
          }
    ...
    <>
      ...
      <ul className={style.ThreeRows}>
        ...
        <li>
          ...
          <input type="number" className="field pointer-events-none" />
        </li>
        <li>
