...
export default function RequestModal() {
  ...
  const rejectRemarksHandler = (button: string, id: number) => {
    setRejectRemarks({
      remarks: "",
      toggle: true,
      button: button,
  ...
  if (isError) {
    return (
      <ModalTemp>
        ...
        <h1 className=" text-ThemeRed text-center">Something went wrong</h1>
      </ModalTemp>
    );
  }
  ...
  return (
    <ModalTemp>
        ...
        <ModalTemp>
          ...
          <div className=" flex justify-between mt-5">
            ...
            <button
              className="buttonRed"
              onClick={() =>
                ActionHandler(rejectRemarks.button, rejectRemarks.id)
              }
            >
      ...
      <ul className="flex flex-wrap text-[14px] 640px:text-[12px] mb-5">
        ...
        <Detail Label={"REQUESTOR"} Value={RequestDetail?.customer_name} />
        <Detail Label={"PROPERTY"} Value={RequestDetail?.property_unit_code} />
        ...
        {RequestDetail?.information?.breif_description !== undefined && (
          <Detail
        ...
        {RequestDetail?.information?.class !== undefined && (
          <Detail Label={"CLASS"} Value={RequestDetail?.information?.class} />
        )}
        ...
        {RequestDetail?.information?.move_in_names && (
          <>
            {RequestDetail?.information?.move_in_names.map(
              (item: { name: string; remarks: string }, index: number) => (
                <li key={index} className="w-full flex flex-wrap">
                  <div className=" w-4/12 640px:w-2/4">
                    <h1 className=" text-ThemeRed uppercase">{"NAME"}</h1>
                    <h4>{item.name}</h4>
                  </div>
                  <div className=" w-4/12 640px:w-2/4">
                    <h1 className=" text-ThemeRed uppercase">{"remarks"}</h1>
                    <h4>{item.remarks}</h4>
                  </div>
                </li>
              )
          ...
          <>
            {RequestDetail?.information?.gate_pass_items.map(
              (
                ...
                <li key={index} className="w-full flex flex-wrap mb-5">
                  <div className=" w-4/12 640px:w-2/4">
                    <h1 className=" text-ThemeRed uppercase">{"QUANTITY"}</h1>
                    <h4>{mapItem.quantity}</h4>
                  </div>
                  <div className=" w-4/12 640px:w-2/4">
                    <h1 className=" text-ThemeRed uppercase">{"ITEM"}</h1>
                    <h4>{mapItem.item}</h4>
                  </div>
                  <div className=" w-4/12 640px:w-2/4">
                    <h1 className=" text-ThemeRed uppercase">{"remarks"}</h1>
        ...
        {isDetails_idcards.length > 0 && (
          <li className="w-full mt-5">
            <h1 className=" text-ThemeRed mb-2">ID CARDS DETAILS</h1>
          </li>
        )}
        ...
        {isDetails_accessCard.length > 0 && (
          <li className="w-full mt-5">
            <h1 className=" text-ThemeRed mb-2">ACCESS CARDS DETAILS</h1>
          </li>
        )}
        ...
        {isDetails_accessCard.map((mapItem, index) => (
          <Detail
            key={index}
            Label={mapItem.label.replaceAll("access_card_details", "")}
            Value={mapItem.value}
          />
        ))}
        ...
        {isToolEquipment.length > 0 &&
          RequestDetail.request === "Amenity Usage" && (
            <li className="w-full mt-5">
              <h1 className=" text-ThemeRed mb-2">TOOLS AND EQUIPMENTS</h1>
            </li>
          )}
          ...
          RequestDetail.request === "Amenity Usage" && (
            <li className="w-full">
              {isToolEquipment.map((mapItem, index) => (
                <h1 className=" uppercase flex items-center" key={index}>
        ...
        {isScopeWork.length > 0 && RequestDetail.request === "Work Permit" && (
          <li className="w-full mt-5">
            <h1 className=" text-ThemeRed mb-2">SCOPE OF WORK</h1>
          </li>
        )}
          ...
          <ul className="max-h-[40vh] 1366px:max-h-[30vh] 640px:max-h-[50vh] overflow-auto">
            {RequestDetail?.request_remarks.map(
              (item: RequestRemarks, index: number) => (
                <RemarksProfile remarkDetail={item} key={index} />
              )
            )}
          </ul>
        </div>
      ...
      <div className="flex justify-end 640px:flex-col">
        ...
        {type === "New Request" && PermissionValidationApprove && (
          <button
            className="buttonRed"
            onClick={() => ActionHandler("In Process", RequestDetail?.id)}
          >
        ...
        {type === "In Process" && (
          <>
            <button
              ...
              onClick={() => ActionHandler("In Review", RequestDetail?.id)}
            >
              {mutateLoading && isButtonClicked === "In Review" ? (
                <ScaleLoader color="#fff" height="10px" width="2px" />
              ) : (
                "REVIEW"
              )}
        ...
        {type === "In Review" && (
          <>
            {PermissionValidationApprove && (
              <button
                className="buttonRed mr-5  640px:mr-0 640px:mb-2"
                onClick={() => ActionHandler("Approved", RequestDetail?.id)}
              >
...
export const Detail = ({ Label, Value }: Detail) => {
  return (
    <li className="w-4/12 640px:w-2/4 mb-5">
      <h1 className=" text-ThemeRed uppercase">{Label.replaceAll("_", " ")}</h1>
      <h4>{Value}</h4>
    </li>
  );
};
  ...
  return (
    <li className="flex  640px:flex-col items-start mb-5">
      <div className="w-[40px] h-[40px] rounded-full overflow-hidden relative 640px:mb-1">
        <Image src={Image_Photo} layout="fill" objectFit="cover" alt="img" />
      </div>
