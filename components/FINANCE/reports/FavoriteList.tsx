...
export default function FavoriteList() {
  ...
  const { data, isLoading, isError } = GetFavoriteList(isReportType, TablePage);
    ...
    <tr onClick={redirect} className=" cursor-pointer">
      ...
      <td>
        ...
        {Project?.map((itemMap: any, index: number) =>
          index === Project.length - 1 ? itemMap.name : itemMap.name + ", "
        )}
      </td>
      <td>
        {(Tower?.length <= 0 || Tower === undefined) && "N/A"}
        {Tower?.map((itemMap: any, index: number) =>
          index === Tower.length - 1 ? itemMap.name : itemMap.name + ", "
        )}
      </td>
      <td>
        {(Floor?.length <= 0 || Floor === undefined) && "N/A"}
        {Floor?.map((itemMap: any, index: number) =>
          index === Floor.length - 1 ? itemMap.name : itemMap.name + ", "
        )}
      </td>
    </tr>
  );
