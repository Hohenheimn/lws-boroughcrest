...
export default function GeneralReportsCheckbox({
  ...
  const SelectHandler = (e: any, column: string, id: number, value: string) => {
    ...
    if (e.target.checked === false) {
      if (column === "document_type") {
        const clone = isDocuments.filter((filterItem) => filterItem.id !== id);
        setDocuments(clone);
      }
      if (column === "Account") {
        const clone = isAccount.filter((filterItem) => filterItem.id !== id);
        setAccount(clone);
      }
    }
  };
            ...
            {GeneralReports.document_type.map((item, index) => (
              <div className="mb-1" key={index}>
                <input
                  ...
                  className="checkbox"
                  onChange={(e) =>
                    SelectHandler(e, "document_type", item.id, item.name)
                  }
                />
                <label htmlFor={`docType_${item.id}`} className={checkBoxLabel}>
                  {item.name}
                </label>
              </div>
            ))}
