...
export const UpdateCollection = (onSuccess: any, onError: any, id: number) => {
  ...
  return useMutation(
    (Payload: any) => {
      return api.put(`/finance/customer-facility/collection/${id}`, Payload, {
        headers: {
          Authorization: "Bearer " + getCookie("user"),
        },
      });
...
export const GetCollectionList = (
  ...
  return useQuery(
    ["collection-list", search, date_from, date_to, page, filterArray, status],
    () => {
      return api.get(
        `/finance/customer-facility/collection?search=${search}&date_from=${date_from}&date_to=${date_to}&paginate=10&page=${
          search === "" ? page : 1
        }&filters=${filterArray}&status=${status}&receipt_type=Acknowledgement,Official`,
        {
          headers: { Authorization: "Bearer " + getCookie("user") },
        }
      );
