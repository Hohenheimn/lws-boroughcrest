...
export default function ReceivePaymentForm({
  ...
  useEffect(() => {
    if (data?.status === 200) {
      ...
      let getCustomerOutstanding = data?.data.map((item: Type_Invoice_list) => {
        let balance = 0;
        if (applyDiscount) {
          balance = Number(item.due_amount) - Number(item.discount_amount);
        } else {
          balance = Number(item.due_amount) - 0;
        }
        Discount_Total = Number(Discount_Total) + Number(item.discount_amount);
        return {
          id: item.id,
          ...
          applied_amount: applyDiscount ? item.discount_amount : 0,
          balance: balance,
    ...
    <>
      {isCancel && (
        <ModalTemp narrow={true}>
          ...
          <div className="flex justify-end items-center w-full">
            <button className="button_cancel" onClick={() => setCancel(false)}>
              NO
            </button>
            <button
              className="buttonRed"
                ...
                inputElement={
                  <input
                    className={`w-full field ${
                      router.query.from === "check_warehouse" && "disabled"
                    }`}
                    value={HeaderForm.receipt_type}
                    readOnly
                  />
                }
                listArray={["Official", "Acknowledgement", "Provisional"]}
              />
            </label>
          </li>
            ...
            <>
                ...
                <label htmlFor="" className="labelField">
                  *MODE&nbsp;OF&nbsp;PAYMENT
                  <SelectDropdown
                    ...
                    inputElement={
                      <input
                        value={HeaderForm.mode_of_payment}
                        className="w-full field"
                        readOnly
                      />
                    }
                  ...
                  />
                  {HeaderForm.mode_of_payment === "" && isErrorToggle && (
                        ...
                        <input
                          ...
                          onClick={() =>
                            setDepositDateRP({
                              ...DepositDateRP,
                              toggle:
                                HeaderForm.mode_of_payment === "Cash"
                                  ? false
                                  : true,
                            })
                          }
              ...
              <li className="w-[30%]  -mt-1">
                <label htmlFor="" className="labelField">
                  *CASH&nbsp;ACCOUNT
                  ...
                  />
                  {HeaderForm.chart_of_account_id === "" && isErrorToggle && (
                    <p className="text-[10px] text-ThemeRed">Required!</p>
                  )}
                </label>
              </li>
                ...
                <label className="labelField flex flex-col">
                  *REFERENCE&nbsp;NO.
                  <input
                    ...
                    disabled={HeaderForm.mode_of_payment === "Cash"}
                    onChange={(e) => {
                      setHeaderForm({
                        ...HeaderForm,
                    ...
                    className={`field w-full ${
                      HeaderForm.mode_of_payment === "Cash" && "disabled"
                    }`}
                  />
                </label>
                {HeaderForm.reference_no === "" &&
              ...
              {HeaderForm.receipt_type === "Official" && (
                <li className="w-[30%]">
                  ...
                  {HeaderForm.credit_tax === "" && isErrorToggle && (
                    <p className="text-[10px] text-ThemeRed">Required!</p>
                  )}
                </li>
              )}
      ...
      {HeaderForm.receipt_type === "Official" && (
        <OfficialForm
          ...
          Error={ErrorToggleHandler}
          headerForm={HeaderForm}
          DefaultOfficialOutrightAdvances={DefaultOfficialOutrightAdvances}
