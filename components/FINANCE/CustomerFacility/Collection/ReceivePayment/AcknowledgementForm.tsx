...
export default function AcknowledgementForm({
  ...
  useEffect(() => {
    setVariance(Number(headerForm?.amount_paid));
  ...
  const SaveHandler = (button: string) => {
    ...
    const Payload = {
      ...
      collection_check_warehouse_id:
        router.query.modify_id === undefined ? null : router.query.modify_id,
    ...
    const closeToUpdate = isTable.map((item: isTableItem) => {
      if (item.id === itemDetail.id) {
        if (keyField === "charge") {
          ...
          const description = value.target.getAttribute("data-description");
          return {
            ...item,
            charge: charge,
            charge_id: charge_id,
    ...
    <tr>
      ...
      <td className="actionIcon">
        ...
        {isTable.length - 1 === index && (
          <div className="ml-5 1024px:ml-2" onClick={(e) => AddRow(e)}>
            <PlusButtonTable />
          </div>
        )}
      </td>
