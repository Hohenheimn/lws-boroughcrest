...
export default function PaymentRegisterDetail({
  ...
  useEffect(() => {
    ...
    CollectionDetail?.outstanding_balances?.map((item) => {
      setOutstandingTotal((prev) => Number(prev) + Number(item.payment_amount));
    });
  ...
  useEffect(() => {
    ...
    CollectionDetail?.check_warehouses?.map((item) => {
      setCheckwareHouseTotal((prev) => Number(prev) + Number(item.amount));
    });
      ...
      <div>
              ...
              {!router.pathname.includes("check-warehouse") && (
                <li className="mr-5">
                  <Tippy theme="ThemeRed" content="Void">
                    <div
                      onClick={() => setToggle(CollectionDetail.id)}
        ...
        <ul className=" flex justify-between relative w-full mb-10 640px:mb-5 flex-wrap">
          ...
          >
            ...
            <div className=" 640px:w-[32%]">
              ...
              <h4 className="main_text">{CustomerDetail?.class}</h4>
            </div>
            <div className=" 640px:w-[32%]">
          ...
          >
            <ul className="flex flex-wrap">
              ...
              <li className="w-[32%]">
                ...
                <h4 className="main_text">{CollectionDetail?.receipt_no}</h4>
              </li>
              <li className="w-[32%]">
                <p className="label_text">RECEIPT TYPE.</p>
                <h4 className="main_text">{CollectionDetail?.receipt_type}</h4>
              </li>
            </ul>
            <ul className="flex flex-wrap">
              <li className="w-full">
                <p className="label_text">DESCRIPTION</p>
                <h4 className="main_text">{CollectionDetail?.description}</h4>
              </li>
            </ul>
            <ul className="flex justify-between flex-wrap space-y-5">
              <li className="w-[32%]">
              ...
              <li className="w-[32%]">
                ...
                <h4 className="main_text">{CollectionDetail?.reference_no}</h4>
              </li>
              <li className="w-[32%]">
        ...
        <ul className=" flex justify-between relative w-full mb-10 flex-wrap">
          {CollectionDetail?.receipt_type === "Official" &&
            CollectionDetail?.outstanding_balances !== undefined && (
              <li
              ...
              >
                ...
                <div className="table_container min-zero border-b border-ThemeRed50 pb-10">
                  <table className="table_list ">
                    ...
                    <tbody>
                      {CollectionDetail.outright_advances !== undefined && (
                        <>
              ...
              >
                ...
                <div className="table_container min-zero border-b border-ThemeRed50 pb-10">
                  <table className="table_list ">
                    ...
                    <tbody>
                      {CollectionDetail?.deposits.map((item, index) => (
                        <tr key={index}>
                          <td>{item.charge_name}</td>
                          <td>{item.description}</td>
                          <td>
              ...
              >
                ...
                <div className="table_container min-zero border-b border-ThemeRed50 pb-10">
                  <table className="table_list ">
                    ...
                    <tbody>
                      {CollectionDetail?.check_warehouses.map((item, index) => (
                        <tr key={index}>
                          <td>{item.check_no}</td>
          ...
          >
            ...
            <div className="flex items-center">
              ...
              <h1 className="main_text noMB">{CollectionDetail?.created_at}</h1>
            </div>
            <div className="flex items-center">
              <p className="label_text  mr-2">Updated at: </p>
              <h1 className="main_text noMB">{CollectionDetail?.updated_at}</h1>
            </div>
          </li>
        </ul>
      </div>
...
const PaymentSummaryList = ({ item }: PaymentSummaryList) => {
  ...
  return (
    <tr>
      ...
      <td>
        <TextNumberDisplay className="w-full withPeso" value={vatAmount} />
      </td>
      <td>
