    ...
    const cloneToPass = isSelectedIDs.map((item) => {
      if (
        ...
        const cloneToFilter = DefaultValue.properties.filter(
          (filterItem) => Number(filterItem.property_unit_id) === item.id
        );
        ...
        <div className={formActive[0] ? "" : "hidden"}>
          ...
          <ul className="mb-5 flex justify-between 640px:flex-col 640px:items-end">
            ...
            <li className="640px:mb-5">
              <input type="file" className=" absolute opacity-0" id="import" />
              <Tippy theme="ThemeRed" content="Import">
                <label className="iconNav" htmlFor="import">
                  <Image src="/Images/Import.png" layout="fill" alt="Import" />
                </label>
              </Tippy>
            </li>
          </ul>
                ...
                {isTableItem?.itemArray.map(
                  (item: isTableItemObj, index: number) => (
                    <TableList
                      ...
                      propertiesvalue={DefaultValue.properties}
                    />
                  )
                )}
              </tbody>
    ...
    propertiesvalue.map((item) => {
      if (item.property_unit_id == itemDetail.id) {
        if (item.modifiable === false) {
          Validate = false;
          setPrompt({
            message: "Property already been tagged, Cannot be removed",
            type: "draft",
            toggle: true,
          });
        }
