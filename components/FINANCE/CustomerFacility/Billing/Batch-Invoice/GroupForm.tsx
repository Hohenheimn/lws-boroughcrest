...
export default function GroupForm({
  ...
  const { data, isLoading, isError } = useQuery(
    ...
    () => {
      return api.get(
        `/admin/customer?keywords=${isSearch}&owner_tenant_class=1&paginate=10&page=${TablePage}&${
  ...
  useEffect(() => {
    const addExistingID = externalDefaultValue?.map((item: CustomerGroup) => {
    ...
    <ModalTemp wide={true}>
      <div>
        ...
        <div className="flex justify-end py-5 mt-10">
          <button className="button_cancel" onClick={() => toggle("")}>
            Cancel
          </button>
