...
export default function JournalForm({
  ...
  const [isBillingFromReading, setBillingFromReading] = useState<billingArray>(
    []
  );
  ...
  const { data, isLoading, isError } = GetInvoiceListByCustomer(isCustomer.id);
  ...
  useEffect(() => {
    if (isCustomer.id !== "") {
      ...
      if (GetInvoiceFromBatch !== undefined) {
        const InvoiceListFromBatch = GetInvoiceFromBatch.map((item: any) => {
          return {
  ...
  const { isLoading: isLoadingSave, mutate: mutateSave } = CreateInvoiceBilling(
    onSuccess,
    onError
  );
      ...
      {deleteToggle && (
        <ModalTemp narrow={true}>
          <h1 className="text-center mb-5">
            Invoice will be deleted and changes is not reversible. Are you sure
            of this action?
          </h1>
          <div className="flex justify-end items-center w-full">
            <button
            ...
            <button className="buttonRed" onClick={() => DeleteHandler()}>
              {deleteLoading ? (
      ...
      <div>
        ...
        <div className="table_container">
          <table className="table_list forCrud">
            ...
            <tbody className={isLoading ? "pointer-events-none" : ""}>
              {/* Not editable */}
              ...
              {/* Editable */}
              {isBilling?.map((item: billingObject, index: number) => (
                <List
                  key={index}
                  index={index}
                  setState={setBilling}
...
const List = ({ itemList, setState, isState, index, isUnitCodes }: List) => {
  ...
  useEffect(() => {
    ...
    const Amount =
      Number(itemList.unit_price) * Number(itemList.quantity) + Number(Vat);
  ...
  const RemoveJournal = () => {
    setState((item: any[]) => item.filter((x: any) => x.id !== itemList.id));
  };
