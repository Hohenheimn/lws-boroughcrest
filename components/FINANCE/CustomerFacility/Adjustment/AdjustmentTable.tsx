...
export default function AdjustmentTable({ type, isPeriod, setPeriod }: Props) {
  ...
  useEffect(() => {
    if (data?.status === 200) {
      ...
      let CloneArray = data?.data?.data.map((item: AdjustmentDetailType) => {
        if (item.id === null) {
          empty = true;
          return;
        }
        ...
        return {
          ...
          properties: item?.customer?.properties.map((prop) => prop.unit_code),
    ...
    <>
      {isRejectNoticeToggle && (
        <ModalTemp narrow={true}>
          <h1 className="text-center mb-5">
            Adjustment will be deleted and changes is not reversible. Are you
            sure of this action?
          </h1>
            ...
            <>
              ...
              <li className={style.importExportPrint}>
                <Tippy theme="ThemeRed" content="Reject">
                  ...
                  >
                    {updateLoading && buttonLoading === "Rejected" ? (
                      <MoonLoader
                        className="text-ThemeRed mr-2"
                        color="#8f384d"
                        size={16}
      ...
      <ul className=" flex flex-wrap">
        {isAdvFilter.map((item, index) => (
          ...
          >
            {item.value} - <span className="text-ThemeRed50">{item.key}</span>
            <span
        ...
        <table className="table_list journal">
          <thead>
            <tr>
              ...
              <th>{type === "unposted" ? "Status" : "Memo No."}</th>
              <th>Date</th>
              <th>Customer</th>
              <th>Property</th>
              <th>Description</th>
        ...
        <div>
          <h2>
            {itemDetail?.properties?.map((item: any, index: number) =>
              itemDetail?.properties?.length - 1 === index ? item : item + ", "
            )}
          </h2>
        </div>
      </td>
