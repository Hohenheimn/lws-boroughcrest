...
export default function AdjustmentForm({ DefaultValue }: Props) {
  ...
  const onSuccess = () => {
    ...
    if (isButton !== "new") {
      router.push("/finance/customer-facility/adjustment/adjustment-list");
    }

    if (isButton === "new" && router.query.modify !== undefined) {
      router.push("/finance/customer-facility/adjustment/create-adjustment");
    }
    ...
    if (invoiceData !== undefined) {
      let getCustomerOutstanding = invoiceData?.data.map(
        (item: Type_Invoice_list) => {
          ...
          let balance =
            Number(item.due_amount) - Number(item?.remaining_advances);

          const date = parse(item.document_date, "yyyy-MM-dd", new Date());
          ...
          return {
            ...
            billing_date: isValid(date) ? format(date, "MMM dd yyyy") : "",
      ...
      setInvoices(
        getCustomerOutstanding === undefined ? [] : getCustomerOutstanding
      );
    }
  }, [invoiceData?.data]);
      ...
      (item: FilteredAccuntEntries, index: number) => {
        ...
        if (userInfo?.corporate_gst_type === "VAT") {
          if (isTransaction === "Charge Debit") {
            if (item.coa_default_account_id === PSReceivableAccount) {
              debit = Number(deferred_customer_gst_account) + Number(less_vat);
            }
            if (item.coa_default_account_id === DeferredCustomerGSTAccount) {
          ...
          if (isTransaction === "Discounts") {
            if (item.coa_default_account_id === DiscountContraAccount) {
              debit = less_vat;
            }
          ...
          if (isTransaction === "Credit Tax") {
            if (item.coa_default_account_id === CustomerWTAXAccount) {
  ...
  const SaveHandler = (button: string) => {
    ...
    if (button !== "draft") {
      ...
      if (isAccounts.some((item) => item.credit === 0 && item.debit === 0)) {
        setPrompt({
      ...
      invoices: AdvancesToggle
        ...
        : isInvoices.map((item) => {
            return {
              ...
              adjustment_amount: Number(item.adjustment_amount).toFixed(2),
          ...
          >
            ...
            <ul className=" flex flex-wrap justify-between w-[75%] 640px:w-full 640px:pl-0 pl-10  pb-10 1550px:pl-5 1550px:pb-5">
              <li className="w-[30%] 640px:w-2/4 pr-5">
                <label htmlFor="" className="labelField">
                  ...
                  <SelectDropdown
                    ...
                    listArray={["Credit Memo", "Debit Memo"]}
                  />
                  {isErrorMessage && HeaderForm.memo_type === "" && (
                    <p className=" text-[12px] text-ThemeRed">Required!</p>
                  )}
                </label>
              </li>
                ...
                <DynamicPopOver
                  toRef={
                    <label className="labelField flex flex-col mt-1">
                      ...
                      {isErrorMessage && HeaderForm.memo_date === "" && (
                        <p className=" text-[12px] text-ThemeRed">Required!</p>
                      )}
                    </label>
                  }
                  toPop={
                    <>
                      {isMemoDate.toggle === true && (
                        <Calendar value={isMemoDate} setValue={setMemoDate} />
                      )}
                    </>
                  }
                  className={""}
              ...
              <li className="w-full flex justify-end">
                <div
                  onClick={() => setAdvancesToggle(!AdvancesToggle)}
            ...
            <div className="table_container hAuto">
              <table className="table_list">
                ...
                <tbody className="textBlack">
                  {isChargeHeader.charge_id !== "" && isCustomer.id !== "" && (
                    <>
                      {refAccEntries?.data.map(
                        (item: AccountEntries, index: number) => (
                          <tr key={index}>
                            <td>{item.document.document_date}</td>
                            <td>{item.document.document_no}</td>
                            <td>{item.charge.description}</td>
                            <td>{item.chart_of_account.chart_code}</td>
        ...
        {!AdvancesToggle && (
          ...
          >
            <InvoiceTransaction
              ...
              ApplyAccountEntriesHandler={ApplyAccountEntriesHandler}
            />
          </motion.div>
        )}
      </AnimatePresence>
