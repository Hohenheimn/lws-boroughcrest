...
export default function JournalForm({
  ...
  const CancelHandler = () => {
    const filter = isJournalList.filter((filter) => filter.account_id !== "");
...
const List = ({ itemList, setDefault, isDefault, index }: List) => {
  ...
  return (
    <tr>
      ...
      <td>
        <DropDownCOA UpdateStateHandler={updateValue} itemDetail={itemList} />
      </td>
      <td>
        <InputNumberForTable
      ...
      <td className="actionIcon flex items-center">
        ...
        {isDefault.length - 1 === index && (
          <div className="ml-5 1024px:ml-2" onClick={(e) => AddJournal(e)}>
            <PlusButtonTable />
          </div>
        )}
      </td>
