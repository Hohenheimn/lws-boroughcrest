...
export default function PrompMessage() {
  ...
  return (
    <motion.div
      ...
      } border transition duration-75 fixed top-10 right-10 480px:top-5 480px:right-5 z-[99999999] 820px:px-7 820px:py-3 opacity-90 px-10 py-5 shadow-lg rounded-lg`}
    >
      {typeof togglePrompt.message === "object" ? (
        <>
          {togglePrompt?.message?.map((item: string, index: number) => (
            <h1 key={index} className="text-white 820px:text-[12px]">
              {item}
            </h1>
          ))}
        </>
